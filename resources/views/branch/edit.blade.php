        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-header bg-warning">
                    <h5 class="card-title text-white mb-0">Form Ubah Cabang</h5>
                </div>

                <!-- Form -->
                <form action="{{ route('branch.update', $dataEditbranch->branch_id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" name="nama_cabang" id="nama_cabang" class="form-control"
                                   value="{{ old('nama_cabang', $dataEditbranch->nama_cabang) }}">
                            @error('nama_cabang')
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="number" name="kontak" id="kontak" class="form-control"
                                   value="{{ old('kontak', $dataEditbranch->kontak) }}">
                            @error('kontak')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea name="alamat" id="alamat" class="form-control" rows="3">{{ old('alamat', $dataEditbranch->alamat) }}</textarea>
                            @error('alamat')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="province_id" id="province_id" class="form-control">
                                ...
                                @foreach($provinces as $id => $name)
                                    <option value="{{ $id }}" {{ old('province_id', $dataEditbranch->province_id) == $id ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-3">
                            ...
                            @php
                                $cities = \Laravolt\Indonesia\Models\City::where('province_code', $dataEditbranch->province_id)->pluck('name', 'code');
                            @endphp
                            <select name="city_id" id="city_id" class="form-control">
                                <option value="">Pilih Kota / Kabupaten</option>
                                @foreach($cities as $code => $name)
                                    <option value="{{ $code }}" {{ old('city_id', $dataEditbranch->city_id) == $code ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-3">
                            ...
                            @php
                                $districts = \Laravolt\Indonesia\Models\District::where('city_code', $dataEditbranch->city_id)->pluck('name', 'code');
                            @endphp
                            <select name="district_id" id="district_id" class="form-control">
                                <option value="">Pilih Kecamatan</option>
                                @foreach($districts as $code => $name)
                                    <option value="{{ $code }}" {{ old('district_id', $dataEditbranch->district_id) == $code ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-3">
                            ...
                            @php
                                $villages = \Laravolt\Indonesia\Models\Village::where('district_code', $dataEditbranch->district_id)->pluck('name', 'code');
                            @endphp
                            <select name="village_id" id="village_id" class="form-control">
                                <option value="">Pilih Kelurahan / Desa</option>
                                @foreach($villages as $code => $name)
                                    <option value="{{ $code }}" {{ old('village_id', $dataEditbranch->village_id) == $code ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
